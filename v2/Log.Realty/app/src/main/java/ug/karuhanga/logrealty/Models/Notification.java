...
import static ug.karuhanga.logrealty.Helper.FLAG_ONE_DAY_AFTER;
import static ug.karuhanga.logrealty.Helper.FLAG_ONE_DAY_TO;
import static ug.karuhanga.logrealty.Helper.REQUEST_CODE_NOTIFY;
import static ug.karuhanga.logrealty.Helper.dateToString;
import static ug.karuhanga.logrealty.Helper.getTodaysDate;
...
public class Notification extends Record {
    ...
    public static boolean schedule(Context context, Tenant tenant, boolean newPayment){
        ...
        if (calendar.getTime().before(getTodaysDate())){
            return schedule(context, tenant, false);
        }
    ...
    public static boolean restore(Context context, Notification notification){
        ...
        if (calendar.getTime().before(getTodaysDate())){
            return schedule(context, tenant, true);
        }
    ...
    public static boolean displayNotification(Context context, String title, String message){
        ...
        notifBuilder.addAction(R.drawable.ic_message_black_24dp, "Message", PendingIntent.getActivity(context, 0, new Intent(context, AddPayment.class), 0));//change to start text
        notifBuilder.addAction(R.drawable.ic_person_add_black_24dp, "Add Payment", PendingIntent.getActivity(context, 0, new Intent(context, AddPayment.class), 0));
